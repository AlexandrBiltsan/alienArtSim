...
namespace BadDog
{
    ...
    public class BGGrassCutter : MonoBehaviour
    {
        ...
        private void CutGrassByType()
        {
            ...
            for (int i = 0; i < terrainList.Count; i++)
            {
                ...
                if (Mathf.Abs(centerPos.y - BGGrassCutUtils.GetWorldHeightOnTerrain(terrain, centerPos)) > maxHeight)
                {
                 ...
                 if(Input.GetMouseButton(1)) //if (cutShape == BGGrassCutShape.Circle)
                {
        ...
        private void Update()
        {
            if(processingType == BGGrassProcessingType.Update && (Input.GetMouseButton(0) || Input.GetMouseButton(1)))
            {
